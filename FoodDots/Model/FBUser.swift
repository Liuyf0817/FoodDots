    ...
    class func currentUser() -> FBUser? {
        ...
        if Auth.auth().currentUser != nil {
            if let dictionary = userDefaults.object(forKey: kCURRENTUSER) {
                return FBUser.init(dictionary as! NSDictionary)
            }
        }
...
func downloadUserFromFirestore(userId: String, email: String, completion: @escaping (_ error: Error?) -> Void) {
    ...
    FirebaseReference(.User).document(userId).getDocument { snapshot, error in
        ...
        if snapshot.exists {
            ...
            let user = FBUser(_id: userId, _email: email, _firstName: "", _lastName: "", _phoneNumber: "")
            saveUserLocally(userDictionary: userdictionaryFrom(user: user) as NSDictionary)
            saveUserToFirestore(fbUser: user)
        }
...
func saveUserToFirestore(fbUser: FBUser) {
    
    FirebaseReference(.User).document(fbUser.id).setData(userdictionaryFrom(user: fbUser)) { (error) in
        
        if error != nil {
            
            print("Error creating fbuser object: ", error!.localizedDescription)
        }
    }
}
...
func userdictionaryFrom(user: FBUser) -> [String: Any] {
...
func updateCurrentUser(withValues: [String: Any], completion: @escaping (_ error: Error?) -> Void) {
    ...
    if let dictionary = userDefaults.object(forKey: kCURRENTUSER) {
        ...
        FirebaseReference(.User).document(FBUser.currentId()).updateData(withValues) { error in
